...
impl <'a> Board<'a> {
    ...
    pub fn make_move(&mut self, x_pos: u8, y_pos: u8, player: &Player) -> MoveResult {
        if x_pos >= 3 || y_pos >= 3 {
        ...
        match position_result {
            Err(()) => {
                Err(MoveError::OutOfBounds(x_pos, y_pos))
            },
            Ok(Some(other_player)) => {
                Err(MoveError::PositionAlreadyFilled(other_player))
            },
            Ok(empty_space) => {
    ...
    fn check_winner_diagonal(&self) -> Option<Winner> {
        if self.0[0][0].is_some() && 
            self.0[0][0] == self.0[1][1] && 
            self.0[1][1] == self.0[2][2] {
            Some(Winner(self.0[0][0].unwrap()))
        } else if self.0[0][2].is_some() && self.0[0][2] == self.0[1][1] && self.0[1][1] == self.0[2][0] {
